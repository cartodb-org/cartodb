...
cdb.admin.FormSchema = cdb.core.Model.extend({
      ...
      for (var k in field.form) {
        ...
        if (f.columns) {
          ...
          for(var i in types) {
            ...
            // Obtain column schema as an array of the following form
            //   [ ['col_name', 'col_name', 'Col Alias'], ... ]
            // or
            //   [ ['col_name', 'col_name', 'col_name'], ... ]
            // if no alias exists for the column.
          ...
          if (self._useAliasesInExtraFix()) {
            var columnNames = self._getColumnNamesFromExtra(extra);
            if (!f.value || !_.contains(columnNames, f.value)) {
              f.value = columnNames[0];
            }
          } else {
            if (!f.value) {
  ...
  _useNewModelComparison: function() {
    if (this._newComparisonEnabled === undefined) {
      this._newComparisonEnabled = _.contains(window.user_data.feature_flags, "bbg_rtr_enable_wizard_prop_comp_fix");
    }
    return this._newComparisonEnabled;
  },

  _useAliasesInExtraFix: function() {
    if (this._aliasesInExtraFixDisabled === undefined) {
      this._aliasesInExtraFixDisabled = _.contains(window.user_data.feature_flags, "bbg_rtr_disable_wizard_aliases_extra_fix");
    }
    return !this._aliasesInExtraFixDisabled;
  },

  // if type is function then returns false if newValue is not any function,
  // for other cases compares values
  _attributeChanged: function(attributeName, newValue) {
    function discardModelAttributes(obj) {
      _.each(obj, function(val, key) {
        if (key == "model") {
          delete obj[key];
        } else if (typeof val === 'object') {
          obj[key] = discardModelAttributes(val);
        }
      });
    ...
    var existing = this.previousAttributes()[attributeName];
    var typeExisting = typeof existing;
    var typeNewValue = typeof newValue;

    if (typeExisting === 'function' && typeNewValue === 'function') {
      return false;
    }

    if (typeExisting === 'object' && typeNewValue === 'object') {
      // deep clone
      existing = jQuery.extend(true, {}, existing);
      newValue = jQuery.extend(true, {}, newValue);
      discardModelAttributes(existing);
      discardModelAttributes(newValue);
    }

    return !_.isEqual(existing, newValue);
  },
  ...
  changedDinamycProperty: function() {
    ...
    var hasChanged;
    for(var i in d) {
      hasChanged = this._useNewModelComparison() ? this._attributeChanged(d[i].name, d[i]) : this.changedAttributes(d[i])
      if (hasChanged) {
        changed.push(d[i]);
      }
    }
  ...
  dinamycValues: function(c) {
    ...
    if (this._useAliasesInExtraFix()) {    
      return this._getColumnNamesFromExtra(v.form[k].extra);
    }
    else {
  ...
  // Extra may store a flat string value e.g. "None"
  // to indicate no selection or an array of names
  // and aliases for column values. In the latter
  // case, the first element i.e. column name is
  // returned.
  _getColumnNamesFromExtra: function(extra) {
    return _(extra).map(function (valueOrArray) {
      return _.isArray(valueOrArray) ? valueOrArray[0] : valueOrArray;
    });
  },

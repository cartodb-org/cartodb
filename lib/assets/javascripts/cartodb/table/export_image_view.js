...
cdb.admin.ExportImageView = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this._insetMapSelector = this.options.insetMapSelector || '.leaflet-control-minimap';
    this._insetMap = this.mapOverlays.getOverlay('inset_map');
    this._insetMapCss = {};
  ...
  render: function() {
    ...
    if (this._insetMap) {
        var $insetMap = $(this._insetMapSelector);
        // Update inset map div's positioning to be consistent for all corners since other
        // overlays are not present. Cache the old positions so they can be restored once the
        // user clears the export overlay
        var exportInsetMapCss = {};
        _.each(['top', 'bottom', 'left', 'right'], function (property) {
            var value = $insetMap.css(property);
            if (value !== 'auto') {
                self._insetMapCss[property] = value;
                exportInsetMapCss[property] = 10;
            }
        });
        $insetMap.css(exportInsetMapCss)
                 .detach()
                 .appendTo(this.$('.CanvasExport'));
    }
  ...
  _showOverlays: function() {
    ...
    this.mapOverlays.overlays.each(function(overlay) {
      ...
      if (type !== "zoom" && type !== "loader" && type !== "annotation" && type !== "text" &&
          type !== "image" && type !== "inset_map") {
  ...
  _resetInsetMap: function () {
    if (this._insetMap) {
        var xPosition = this._insetMap.get('xPosition');
        var yPosition = this._insetMap.get('yPosition');
        $(this._insetMapSelector)
          .css(this._insetMapCss)
          .detach()
          .appendTo('.cartodb-map .leaflet-' + xPosition + '.leaflet-' + yPosition);
    }
  },
